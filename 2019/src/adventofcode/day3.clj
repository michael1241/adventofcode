...
(def path1 (clojure.string/split path1 #","))
(def path2 (clojure.string/split path2 #","))

(defn direc
  "takes coord difference; returns direction of line"
  [diff]
  (if (pos-int? diff) 1 -1))
...
(defn covered-points
  ...
  (let [diff (- (nth end plane) (nth start plane))]
    (let [changecoords (range (nth start plane) (+ (nth end plane) (direc diff)) (direc diff))
          samecoords (repeat (+ (Math/abs diff) 1) (nth start (invert plane)))]
      (if (== plane 0)
        (map vector changecoords samecoords)
...
(defn record-path
  ...
  (loop [start [0 0]
        ...
        path (rest path)
        traversed #{}]
    (let [line (define-line start step)]
      (if (empty? path)
        (reduce conj traversed line)
        (recur (last line) (first path) (rest path) (reduce conj traversed line))))))

(defn find-intersect
  "takes two lists of instructions and returns set of intersecting coords"
  [path1 path2]
  (let [set1 (record-path path1)
        set2 (record-path path2)]
    (clojure.set/intersection set1 set2)))

(defn intersect-distance
  "takes coord and returns manhattan distance from origin"
  [coords]
  (+ (Math/abs (nth coords 0)) (Math/abs (nth coords 1))))

(defn closest-intersect
  "takes set of intersections; returns intersection closest to origin"
  [path1 path2]
  (let [intersects (seq (disj (find-intersect path1 path2) [0 0]))]
    (apply min (map intersect-distance intersects))))
